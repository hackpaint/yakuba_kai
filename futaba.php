...
/* -------------------------------------------------------------------------
 画像掲示板(ふたば・しおから系)
futaba.php v0.8 lot.041107 Yakuba改 20170319版
----------------------------------------------------------------------------
■futaba.phpについて
...
■yakuba版について
このスクリプトは上記のスクリプトをさらにいろいろ改造したものです。
詳細はreadme.txtにあります。

いろいろ注意事項がありますのでreadme.txtは必ずお読みください。
お問合せは隠れ里の村役場(http://t-jun.kemoren.com/)まで。
------------------------------------------------------------------------- */
...
// define("HOME_URL", "");                                 // ホームurl(これがない場合、ホームなどのリンクが相対指定になります。最後は/で終わること。)
// define("HTMLADD", 0);                                   // 返信モードのurlをhtmlに(しない：0、する：1)(する場合、htaccessでの操作が必要です。)
// 上記２つは未実装。
// ▲Yakuba追加
//=============================================================================
...
function form(&$dat,$resno,$admin="",$name_flag=FALSE){
  ...
  $no = $resno;
  if($resno){
//    $msg .= "[<a href=\"".HOME_URL..PHP_SELF2."\">掲示板に戻る</a>]\n";       ◆ yakuba改
...
//         $j = mb_convert_encoding($j, "UTF-8", "SJIS");                       ◆yakuba改 SJISじゃなくてもUTF-8に変換(20190527)
        $c_name.=$j;
//        $c_name.="%u".bin2hex($j);                                            ◆yakuba改 クッキーの名前がなぜかバイナリの10進数になってたので訂正(20190527)
        $i++;
      }
